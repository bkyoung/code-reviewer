...
| **2: GitHub Native** | v0.3.x | 🚧 In Progress | First-class reviewer, inline annotations |
...
| 2.2 | Review API Integration | ✅ Complete |
| 2.3 | Request Changes | ✅ Complete |
| 2.4 | Skip Trigger (`[skip code-review]`) | Not Started |
| 2.5 | Incremental Reviews | 🚧 In Progress |
| 2.6 | Finding Deduplication | 🚧 In Progress |
| 2.7 | PR Size Guards | Not Started |

### Architecture Note: Platform Adaptability

Milestones 2.5 and 2.6 are unified in **Epic #53** with a clean architecture that separates:
- **Domain Layer:** Finding fingerprinting, status model (platform-agnostic)
- **Use Case Layer:** Deduplication logic, incremental diffing (platform-agnostic)
- **Adapter Layer:** Storage varies by platform (GitHub comment vs SQLite for CLI)

This ensures future CLI parity and multi-platform support (GitLab, Bitbucket) without rewriting core logic.

**Exit Criteria:**
- [x] Bot initiates GitHub code reviews (not just comments)
- [x] Findings appear as inline annotations on specific lines
- [x] Configurable rules trigger "Request Changes"
